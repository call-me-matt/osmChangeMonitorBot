...
from telegram import error, ReplyKeyboardRemove
...
class telegramHandler(threading.Thread):
    ...
    def cancel(self, update, context):
        ...
        update.message.reply_text(_("Bye!"), reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    ...
    def getOsmChanges(
        self, context: CallbackContext, osmUsers=databaseHandler.getOsmUsers()
    ):
    ...
    def __init__(self):
        ...
        job_queue = JobQueue()
        self.app = Application.builder().token(TOKEN).job_queue(job_queue).build()

        logger.debug("initializing registration database")
        databaseHandler.init()

        logger.debug("creating stats-watchdog")
        watchdog = self.app.job_queue.run_repeating(
            self.getOsmChanges, interval=WATCHDOG_INTERVAL_MIN * 60, first=30
        )
        # watchdog = self.app.job_queue.run_daily(self.getOsmChanges, time(hour=12, minute=00))
    ...
    def run(self):
        ...
        self.app.add_handler(stop_handler)
        ...
        self.app.add_handler(follow_handler)
        ...
        self.app.add_handler(unfollow_handler)
